...
def fill_all_category_sets():
    ...
    for ability_key in pokemon_abilities_json.keys():
        ...
        ability_name = pokemon_abilities_json[ability_key]['name'].lower()
    ...
    for item_key in pokemon_items_json.keys():
        ...
        item_name = pokemon_items_json[item_key]['name'].lower()
    ...
    for attack_key in attacks_json.keys():
        ...
        attack_name = attacks_json[attack_key]['name'].lower()
...
def sim_pokemon_from_json(sim_data):
    lookup_details = sim_data['details']
    #strip shiny
    lookup_details = lookup_details.replace(', shiny', '')
    lookup_details = lookup_details.split(', ')
...
#            print('atk_in_name',atk_in_name)
    ...
    pokemon.curr_health = float(curr_health)
    pokemon.health_ratio = float(curr_health)
    pokemon.atk = atk
    pokemon.spatk = spatk
    pokemon.defense = defense
    pokemon.spdef = spdef
...
def get_pokemon_with_pp_applied(pokemon, seen_data):
    ...
    pokemon = copy_pokemon(pokemon, keep_attacks=True)

    # seen data posses info such as current form
    if pokemon.name != 'unregistered_pokemon' and seen_data != None:
        #Update revealed stats
...
def copy_pokemon(pk_data, keep_attacks=False):
    pokemon = Pokemon()
    ...
    if keep_attacks:
        pokemon.attacks = pk_data.attacks
    return pokemon

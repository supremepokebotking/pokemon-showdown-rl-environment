...
class State:
    ...
    def encode_field_state(self, is_p1_perspective):
        ...
        if not is_p1_perspective:
            seen_attacks_encodes = [
                all_pokemon_attacks_labels.transform([id_for_attack_name(self.p2_seen_attacks['a'])]),     # category
                all_pokemon_attacks_labels.transform([id_for_attack_name(self.p2_seen_attacks['b'])]),     # category
            ]
    ...
    def get_observation(self, includeTranscript=False):
        ...
        results = {
            ...
            'p1_reward':self.p1_reward,
            'p2_reward':self.p2_reward,
...
#        targets[np.nonzero(targets==0)] = -4000000
    ...
    def encode_single_pokemon_state(self, pokemon, show_full_details=False):
        ...
        category_encode = [
        #TODO add gmax forms to json
            all_pokemon_names_labels.transform([pokemon.form.replace('-Gmax', '')]),     # category
...
#        self.summary_printout()
        ...
        if p1_action in ATTACK_ACTIONS:
            self.p1_reward += attack_reward
        if p2_action in ATTACK_ACTIONS:
            self.p2_reward += attack_reward

        #penalty for switching if not forced
        if p1_action in SWITCH_ACTIONS and self.p1_must_switch['a'] == False:
            self.p1_reward -= switch_penalty
        if p2_action in SWITCH_ACTIONS and self.p2_must_switch['a'] == False:
...
#|faint|p1a: Hawlucha
                ...
                fainted_reward = 75
                if is_player_1:
                    self.p1_reward -= fainted_reward
                    self.p2_reward += fainted_reward
...
#        p1_action = get_sample_action('p1', self._state.sample_actions(position='a', is_p1_perspective=True))
        ...
        info = {"transcript":obs['transcript'], "valid_onehot_player_1": obs['valid_onehot_player_1'],
         ...
         "p1_reward": obs['p1_reward'],
         "p2_reward": obs['p2_reward'],
...
#        print('valid_onehot', obs['valid_onehot_player'])
        return obs['combined'], self.reward(reward), done, info
